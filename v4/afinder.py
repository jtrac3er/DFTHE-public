"""
Finde C2enpaare, die erfüllen, dass q1**2 == q1

Und was habe ich herausgefunden? Es sieht so aus, als ob C2en wirklch ähnlich sind wie
Komplexe Zahlen, auf jeden Fall, vielleicht gibt es noch Annomalien, aber was sicher ist

 - für jede Primzahl p gibt es sehr viele C2enpaare q/q' welche die Gleichungen erfüllen:
 	q + q' = QUnit
 	q * q' = QZero 
...
C2en sind ja nicht mal kommutativ, aber naja, keine Ahnung wie das alles zusammenhängt. Vielleicht gibt es 
...
def c2_gen(N, nonzero=False):
	for liste in product(range(N), repeat=4):
		res = C2(*list(liste))
		if nonzero and res == QZero: continue
		yield res

def c2_gen_random(N, k):
	for i in range(k):
		yield C2(*[randint(0,N) for j in range(4)])
